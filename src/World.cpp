...
std::shared_ptr<WorldOutput> World::generateWorld(std::optional<int> seed)
{
...
void World::generateBase(Grid<Tile>& grid) const
{
...
void World::addGrass(Grid<Tile>& grid) const
{
	float scale = generation_data.scale;
	for (int x = 0; x < world_width_tiles; x++)
	{
...
void World::addDirt(Grid<Tile>& grid) const
{
	float scale = generation_data.scale;
	for (int x = 0; x < world_width_tiles; x++)
	{
...
void World::addCaves(Grid<Tile>& grid) const
{
	float scale = generation_data.scale;
...
void World::removeTileCave(const glm::ivec2& position, Grid<Tile>& grid) const
{
...
void World::applyCellularAutomata(Grid<Tile>& grid)
{
...
void World::addWater(Grid<Tile>& grid) const
{
	float scale = generation_data.scale;
	for (int x = 0; x < world_width_tiles; x++)
	{
...
void World::addBiomes(Grid<Tile>& grid) const
{
	float scale = generation_data.scale;
	for (int x = 0; x < world_width_tiles; x++)
	{
...
void World::addObjects(std::vector<ObjectData>& objects, Grid<Tile>& grid)
{
	std::mt19937 rng(master_seed);
	float scale = generation_data.scale;
	for (int x = 0; x < world_width_tiles; x++)
...
void World::addChests(std::vector<ChestData>& chests, Grid<Tile>& grid)
{
	std::mt19937 rng(master_seed);
