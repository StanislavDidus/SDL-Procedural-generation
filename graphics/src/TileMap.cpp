...
graphics::TileMap::TileMap(std::shared_ptr<SDL_GPUDevice> device, std::shared_ptr<GpuTexture> texture, int world_width_tiles, int world_height_tiles,
                           int tile_width_world, int tile_height_world, int chunk_width_tiles, int chunk_height_tiles, float offset_x, float offset_y)
		...
		, size{world_width_tiles * world_height_tiles}
{
	...
	for (int y = 0; y < world_height_tiles; ++y)
	{
		for (int x = 0; x < world_width_tiles; ++x)
		{
			...
			data[index] = TileData
			{
				.position = {offset_x + x * tile_width_world, offset_y + y * tile_height_world, 0.0f, 0.0f},
